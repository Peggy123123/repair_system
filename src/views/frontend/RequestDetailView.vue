<template>
  <div class="container py-2">
    ...
    <div v-else class="flex flex-col gap-3">
      ...
      <!-- 補充描述歷史卡片 -->
      <div v-if="request.supplements && request.supplements.length > 0" class="bg-white shadow rounded-lg overflow-hidden">
        <div class="px-4 py-5 sm:p-6">
          <h4 class="text-lg font-medium text-gray-900 mb-4">補充描述</h4>
          <div class="space-y-4">
            <div
              v-for="supplement in request.supplements"
              :key="supplement.id"
...
const currentSupplementImages = computed(() => {
  if (!currentSupplementId.value || !request.value?.supplements) return []
  const supplement = request.value.supplements.find(s => s.id === currentSupplementId.value)
...
const handleSupplementFileUpload = (event: Event) => {
  ...
  if (files && files.length > 0) {
    ...
    if (remainingSlots <= 0) {
      alert(`最多只能上傳 ${MAX_SUPPLEMENT_IMAGES} 張圖片`)
      return
    }
...
const confirmAddDescription = () => {
  ...
  // 使用工具函數添加使用者描述（包含圖片）
  addUserDescriptionToRequest(
    repairRequestsStore.requests,
    request.value.id,
